...
- Added a `cursor` variant to the [Spinner Component](/components/spinner)
...
- Adds new `divider-` variants to the [Table Component](/components/table)
- Adds the new [Mark Component](/components/mark)

### Migration Guide from 0.1.8

Tables no longer have a border surrounding them. Use the `box-` utility to re-add the surrounding border.

## 0.1.8

- Applies `word-break: break-all` to `html, body` **NOTE: you may get some unexpected behavior**
- Adds initial values for the progress component so that it works with lightningcss
- Fixes table borders (yet again)
- Adds a `variant-` attribute to the [Separator Component](/components/separator)
- Adds a `variant-` attribute to the [Table Component](/components/table)
- Adds a `round-tree` marker variant to the [List Component](/components/typography#ul-markers)
- Adds a `--list-marker-color` CSS variable to the [List Component](/components/typography#ul-markers)
- Adds a `--blockquote-indent-color` CSS variable to the [Blockquote Component](/components/typography#blockquote)

## 0.1.6 - 0.1.7

- Properly display the [Table Component](/components/table) when it has a
  single row
- Adds the [Accordion Component](/components/accordion)
- Updated the [Progress Component](/components/progress) to use CSS variables

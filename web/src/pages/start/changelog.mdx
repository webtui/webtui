...
Prior to this release, the `--box-border-color` CSS variable **always** defaulted to `--foreground0`.

Now all child elements of an element with the `--box-border-color` CSS variable will inherit the value
...
The default box border color can also be controlled from `:root`
